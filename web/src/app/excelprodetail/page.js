...
class ExcelDetailPage extends Component {
  ...
  handleNew() {
    const myPayload = {};
    this.postRequest('/api/excelpro/loadnew', null, null, this.sucessCallBackLoadNew, myPayload);
  }

  handleLoad(id) {
    const myPayload = { id: id,};
    this.postRequest('/api/excelpro/load', null, null, this.sucessCallBackLoad, myPayload);
  }

  handleTemplateChange() {
    const myPayload = this.state.main_data;
    this.postRequest('/api/excelpro/templatechange', this.startLoading, this.stopLoading, this.sucessCallBackTemplateChange, myPayload);
  }

  handleClone(id) {
    const myPayload = { id: id,};
    this.postRequest('/api/excelpro/load', null, null, this.sucessCallBackClone, myPayload);
  }

  handleSavePOC = (event) => {
    this.postRequest('/api/excelpro/save', this.startLoading, this.stopLoading, this.sucessCallBackSave, this.state.main_data);
  };
  ...
  handleSubListValueChange = (event, name, index) => {
    ...
    if (fieldIndex !== -1) {
      this.setState((prevData) => {
        const value = event.selectedItem;
        const newData = { ...prevData };
        newData.main_data.fields[fieldIndex].list[index].uploaded_column = value;
        return newData;
      });
    }
  };
  ...
  handleOperationListValueChange = (event, name, index) => {
    ...
    if (fieldIndex !== -1) {
      this.setState((prevData) => {
        const value = event.selectedItem;
        const newData = { ...prevData };
        newData.main_data.fields[fieldIndex].list[index].operation_value = value;
        return newData;
      });
    }
  };
                        ...
                        {this.state.main_data && (
                            <div className="my-component">
                              <div className="fin-header-section">
                                ...
                                <div className="fin-text-heading-label">Enter the excel template information </div>
                              </div>
