...
class DraggableList extends Component {
  ...
  moveItem = (dragIndex, hoverIndex) => {
    ...
    {items.map((item, index) => (
      item.text === itemsCompare[index].text ? (item.difference = false ) : ( item.difference = true)
    ))}
  ...
  render() {
    ...
    const Card = ({ id, text, index, difference }) => {
      ...
      return (
          <span>
            { difference ? (
              <div ref={(node) => ref(drop(node))} style={{ padding: '4px', width: '300px', border: '1px solid #D21404', marginBottom: '4px' }}>
              {text} 
              </div>
            ):(
              <div ref={(node) => ref(drop(node))} style={{ padding: '4px', width: '300px', border: '1px solid #ccc', marginBottom: '4px' }}>
              {text} 
            </div>
            )}
          </span>
    ...
    return (
      <div>
        {items.map((item, index) => (
          <Card key={item.id} id={item.id} text={item.text} index={index} difference={item.difference} />
        ))}
      </div>
    );
  }
      ...
      <DndProvider backend={HTML5Backend}>
        <div className="outer-container">
          ...
          <div className="right-side">
            ...
            <DraggableList data={this.props.data2} dataCompare={this.props.data1} />
          </div>
        </div>
      </DndProvider>
    );
